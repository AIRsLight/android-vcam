 ...
 */
#define LOG_TAG "VirtualCameraStandaloneModule"
...
class StandaloneModule final {
  public:
    StandaloneModule() {
        cameras_[0] = std::make_unique<VirtualCamera>(
                0, 1, true, kVcamClientPackageTag);
        cameras_[1] = std::make_unique<VirtualCamera>(
                1, 1, true, kVcamClientPackageTag);
    }
    ...
    int open(const hw_module_t* module, const char* name, hw_device_t** device) {
        ...
        char* end = nullptr;
        const long id = strtol(name, &end, 10);
        if (end == nullptr || *end != '\0' || id < 0 ||
                id >= static_cast<long>(cameras_.size())) {
            return -EINVAL;
        }
        return cameras_[id]->open(module, device);
    }
...
int setCallbacks(const camera_module_callbacks_t* callbacks) {
    return callbacks == nullptr ? -EINVAL : 0;
}
...
void getAllVendorTags(const vendor_tag_ops_t*, uint32_t* tags) {
    if (tags != nullptr) tags[0] = kVcamClientPackageTag;
}

const char* getVendorSectionName(const vendor_tag_ops_t*, uint32_t tag) {
    return tag == kVcamClientPackageTag ? kVcamClientPackageSection : nullptr;
}

const char* getVendorTagName(const vendor_tag_ops_t*, uint32_t tag) {
    return tag == kVcamClientPackageTag ? kVcamClientPackageName : nullptr;
}

int getVendorTagType(const vendor_tag_ops_t*, uint32_t tag) {
    return tag == kVcamClientPackageTag ? TYPE_BYTE : -1;
}
...
camera_module_t HAL_MODULE_INFO_SYM __attribute__((visibility("default"))) = {
        .common = {
                ...
                .name = "android-vcam standalone Camera3 module",
