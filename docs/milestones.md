...
| --- | --- | --- | --- |
| 10 | HIDL Provider 2.4 / Camera HAL3 | common OnePlus global shim, then version-pinned CameraService for app scope | API 29 ARM64 global shim builds and the OnePlus 7 Pro firmware passes the static HMI/module gate; hardware qualification and 32-bit app scope remain pending |
| 11 | HIDL Provider 2.4 plus concurrent-camera APIs | common OnePlus global shim, then Android 11 ID filtering and concurrency tests | API 29 ARM64 global shim builds and the OnePlus 8 firmware passes the static HMI/module gate; hardware qualification and 32-bit app scope remain pending |
...
| 14 | stable AIDL v2 with OEM variants | AIDL v2 frontend, CameraService integration and product policy | AIDL frame delivery, undeclared notification, FCM 8 declaration, internal IDs 1000/1001, local/RTSP target writes, final-resolution YUV output, packaged manager-independent backend, bounded late-network retry, ordinary Camera2 coexistence and automatic next-boot rollback qualified on NX769J. Segmented public-to-internal ID rewriting, CaptureRequest repair, listener/status filtering, death-notification proxying and remove-listener identity preservation pass AOSP r23 host tests and an NX769J cold boot. The API 34 AOSP x86_64 AVD qualifies the same-process launcher, AIDL v2 provider, public-ID filtering, automatic Camera2/Camera1 topology-map generation, fail-closed pre-publication behavior, and a global ID 10 to internal 1000 color-bar preview through Camera1, Camera2 and NDK under SELinux Enforcing. The dedicated VCAM data type also passes an `android-14.0.0_r23` product `vendor_sepolicy.cil` build. Schema 7 now classifies unknown API 34 builds as non-authorizing, global-only probe candidates and requires reviewed promotion after pass-through protocol/topology evidence. A separate `skip_mount` report-only module packages this probe without native payloads, partition overlays, SELinux rules or camera activation. Automatic systemless policy delivery, broader third-party coverage and additional OEM integration remain pending |

The OnePlus global baseline is intentionally separate from app-scoped routing.
It remains one 64-bit Provider/HAL adapter on Android 10–14 even where
